...
function _projection(Inputs, d3) {
  return (
...
function _height(d3, projection, width, sphere) {
  ...
  projection.scale(projection.scale() * (l - 1) / l).precision(0.1);
  return dy;
}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["land-110m.json", { url: new URL("./files/simple.json", import.meta.url), mimeType: "application/json", toString }],
    ["land-50m.json", { url: new URL("./files/detail.json", import.meta.url), mimeType: "application/json", toString }]
  ]);
